...
const shutdown = (...reasons) => {
  console.error('Kue shutdown:', ...reasons);
...
const promiseMaybeArray = (obj, getPromise) => {
  if( Array.isArray(obj) ) {
    return Promise.props(obj.reduce((memo, item) => {
      memo[item] = getPromise(item);
      return memo;
    }, {}));
  }
...
class JobQueueService {
   ...
   * @param {boolean} [options.zone=true] Whether to sandbox job functions with Zone.js
   *
   * @listens jobs start
  ...
  setup(app) {
    ...
    // if( !this.options.zone && this.options.shutdownOnException ) {
    if( this.options.shutdownOnException ) {
      process.once('uncaughtException', ex => {
        shutdown('uncaught exception', ex);
      });
    }
   ...
   *
   * zone:
   *   If this exists and is truthy, the job will be zoned. This can be overridden per-job
   *
  ...
  loadJobDefinitions() {
    ...
    fs.readdirSync(this.options.jobDefinitionPath).forEach(file => {
      ...
      // Zone the function if requested.
      // if( (!('zone' in this.jobDefs) && this.options.zone) || this.jobDefs.zone ) {
      //   jobExports.jobFunction = zone.define(function (job, ctx, done) {
      //     zone.setCallback(done);
      //     jobExports.jobFunction(job, ctx, zone.return);
      //   });
      // }
  ...
  find(params) {
    ...
    let promise;
    if( query.type ) {
      promise = promiseMaybeArray(query.type, type => this.findByType(type, query));
    } else if( query.state ) {
      promise = promiseMaybeArray(query.state, state => this.findByState(state, query));
    }
  ...
  create(data) {
    ...
    promise = promise.then(() => {
      ...
      return new Promise((resolve, reject) => {
        job.save(err => {
          if( err ) { this.debug('Job creation: Error when saving job!', err); reject(err); return; }
          this.debug(`Job creation: success, with id ${job.id}!`); resolve(job.id);
        });
      });
    });
